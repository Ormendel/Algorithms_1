...
public class median 
{
	...
	/**
	 * There is a given array; we need to find a number that is bigger than the median. the
	 ...
	 * 1-1/(2^64) which is 1.
	 * 
	 * the array is not necessary sorted!
	 * 
	 * @param arr
	 * @return
	 */
	...
	{//Problem B
		...
		//Assuming A and B are both n x n
		int n = A.length;
		int i = 0;
		int j = n-1;
		int k = n-1;
		...
		while(k >= 0) {
			if(A[i] >= B[j])
			{
				//According to the transitive principle, A[i] is bigger from all B's values
