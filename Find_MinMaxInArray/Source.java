...
public class Source
{
	...
	public static int[] GetMinMax(int arr[]) 
	{
		...
		if (arr[0] < arr[1])
		{
			// making sure our guess was right
			min = arr[0];// if not then switch
			max = arr[1];
		}
		for (int i = 2; i < arr.length; i += 2)
		{
			// compare between every pair
			comparisons++;
		...
		if (arr.length % 2 != 0)
		{
			// if size of array is odd so we need to check the last element
			comparisons++;
	...
	int ans[] = { min, max, comparisons };//Displaying our answer as array
		return ans;
	...
	static int comparisons = 0;
		...
		public static void Get2Max(int[] a)
		{
			...
			while (!arr[index].st.isEmpty())//The second maximum will be in the stack of the first maximum
			{
